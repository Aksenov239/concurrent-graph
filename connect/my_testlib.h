...
#define ensure(cond) if (!(cond)) quitf(_fail, "assertion failed, line: %d, condition: %s", __LINE__, #cond)
#define quit quitf

void quitf( TResult result, const char *msg, va_list list );
...
struct InStream
{
  ...
  void quitf( TResult result, const char *msg, ... ) const
  {
    va_list list;
    va_start(list, msg);
    ::quitf(mode == _output ? result : _fail, msg, list);
  }
...
inline int InStream::readChar( byte c )
{
  int res = readChar();
  if (res == -1)
    this->quitf(_pe, "Unexpected end of file");
  else if (res != c)
    this->quitf(_pe, "Expected char '%c', but '%s' found", c, outputChar(res));
  return res;
}
...
{
  if (!eoln())
    this->quitf(_pe, "Expected EOLN");
}

inline void InStream::readEof()
{
  if (!eof())
    this->quitf(_pe, "Expected EOF");
}
    ...
    while (isdigit(c))
    {
      ...
      if (++len >= 19)
      {
        ...
        if ((sign == 1 && t < 0) || (sign == -1 && t < -1))
          this->quitf(_pe, "64-bit signed integer overflow");
      }
      c = readChar();
    }
    if (zeroes && zeroes + len > 1)
      this->quitf(_pe, "Leading zeroes are not allowed [zeroes = %d, len = %d]", zeroes, len);
  }
  else
    this->quitf(_pe, "Expected integer, but '%s' found", outputChar(c));
  unreadChar(c);
  x *= sign;
  if (!(l <= x && x <= r))
    this->quitf(_pe, "parse integer: out of range ["I64".."I64"]", l, r);
  return x;
}
